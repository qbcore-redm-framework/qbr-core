...
function QBCore.Player.CreatePlayer(PlayerData)
	...
	self.Functions.AddXp = function(skill, amount)
		...
		if self.PlayerData.metadata['xp'][skill] and amount > 0 then
			...
			self.Functions.UpdatePlayerData()
			TriggerEvent('qb-log:server:CreateLog', 'levels', 'AddXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** has received: '..amount..'xp in the skill: '..skill..'. Their current xp amount is: '..self.PlayerData.metadata['xp'][skill])
			return true
		end
		return false
	end
	...
	self.Functions.RemoveXp = function(skill, amount)
		...
		if self.PlayerData.metadata['xp'][skill] and amount > 0 then
			...
			self.Functions.UpdateLevelData(skill)
			self.Functions.UpdatePlayerData()
			TriggerEvent('qb-log:server:CreateLog', 'levels', 'RemoveXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** was stripped of: '..amount..'xp in the skill: '..skill..'. Their current xp amount is: '..self.PlayerData.metadata['xp'][skill])
			return true
		end
		return false
	end
