...
local function CreatePlayer(PlayerData)
    ...
    self.Functions.AddItem = function(item, amount, slot, info)
        ...
        if itemInfo == nil then
            TriggerClientEvent('QBCore:Notify', self.PlayerData.source, 9, Lang:t('error.item_not_exist'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            return
        end
        local amount = tonumber(amount)
        ...
        if (totalWeight + (itemInfo['weight'] * amount)) <= QBConfig.Player.MaxWeight then
            ...
            TriggerClientEvent('QBCore:Notify', self.PlayerData.source, 9, Lang:t('error.too_heavy'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
        return false
    end
